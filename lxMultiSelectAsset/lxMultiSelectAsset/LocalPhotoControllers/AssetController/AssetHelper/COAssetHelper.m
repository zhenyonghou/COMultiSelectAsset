...
@implementation COAssetHelper

static COAssetHelper *_sharedInstance;

+ (COAssetHelper *)sharedInstance
{
    @synchronized (self){
        if (_sharedInstance == nil){
            _sharedInstance = [[COAssetHelper alloc] init];
...
- (UIImage *)getImageFromAsset:(ALAsset *)asset type:(NSInteger)nType
{
    ...
    if (nType == kAssetPhotoSizeTypeThumbnail)
        iRef = [asset thumbnail];
    else if (nType == kAssetPhotoSizeTypeScreen)
        iRef = [asset.defaultRepresentation fullScreenImage];
    else if (nType == kAssetPhotoSizeTypeResolution)
    {
